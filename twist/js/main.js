;(function () {
	...
	var clickMenu = function() {
  	...
  	// custom - inline link nav
	var customLinkClick = function() {

		$('.scrollableLink a:not([class*="external"])').click(function(event){
			var section = $(this).data('nav-section'),
				navbar = $('#navbar');
		...
		if ( how.length > 0 ) {	
			...
			how.waypoint( function( direction ) {
				...
				if( direction === 'down' && !$(this.element).hasClass('animated') ) {
					...
					setTimeout(function() {
						how.find('.to-animate').each(function( k ) {
							var el = $(this);
					...
					setTimeout(function() {
						how.find('.to-animate-2').each(function( k ) {
							var el = $(this);
